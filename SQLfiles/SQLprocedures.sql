...
DROP PROCEDURE IF EXISTS delete_filler_dates;
DELIMITER $$
...
BEGIN
	...
	WITH grouped AS(
		SELECT
			time_stamp,
			track_id,
			SUM(is_new_group) OVER (ORDER BY time_stamp ROWS UNBOUNDED PRECEDING) AS group_id
		FROM listening_two
	),
    minmax AS(
        SELECT group_id, MIN(time_stamp) AS mi, MAX(time_stamp) AS ma FROM grouped GROUP BY group_id)


    DELETE l
    FROM listening_two AS l
    WHERE l.time_stamp not in (SELECT mi FROM minmax UNION ALL SELECT ma FROM minmax);
--    JOIN minmax m
--    ON l.time_stamp <> m.mi
--    AND l.time_stamp <> m.ma;
...
BEGIN
    ...
    #CREATE TABLE json_tracks AS (SELECT * FROM playlist_tracks LIMIT 0);
